package net.darkhail.mccourse.worldgen.tree.custom;
...
public class WalnutFoliagePlacer extends FoliagePlacer {
    public static Codec<WalnutFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance)
    ...
    protected void createFoliage(LevelSimulatedReader pLevel, FoliageSetter foliageSetter, RandomSource pRandom, TreeConfiguration pConfig, int maxFreeTreeHeight,
        ...
        //attachment.pos() is the first poistion ABOVE the last places log

        //tryPlaceLeaf() // places on leave at given position
        for(int i = 0; i < 4; i++) {
            this.placeLeavesRow(pLevel, foliageSetter, pRandom, pConfig, attachment.pos().above(i), 1, i + 1, attachment.doubleTrunk());
        }
